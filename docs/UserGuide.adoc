...
Select a class by its index in the full class list. You can see the full class list by using `class list`. +

Format: `class delete INDEX` +
Example: `class delete 1` +

Select a class by its index in the module's class list. +
...
|  `*class* find /at DAY`
|  `*class* find /next`
|  `*class* edit INDEX [/code MOD_CODE] [/type CLASS_TYPE] [/at DAY START_TIME END_TIME] [/venue VENUE]`
|  `*class* delete INDEX`
