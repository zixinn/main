...
* A `Facilitator` also contains at least one `ModuleCode` to indicate which `Module` the facilitator belongs.
...
* A `Lesson` contains a `DayAndTime` which indicate which day of the week and what time the lesson is on.
* A `Lesson` also contains a `LessonType` and a `ModuleCode` to indicate which `Module` the lesson belongs.
...
==== Calendar
* The `Calendar` package contains a concrete class `Calendar`.
* The `Calendar` contains a LocalDate to represent the calendar date.
...
* `find` - Finds a facilitator in Mod Manager by name.
...
=== Use case: UC15 - View all tasks across modules in Mod Manager
*<<mss, MSS>>*
[none]
...
=== Use case: UC16 - View tasks for a specific module in Mod Manger
