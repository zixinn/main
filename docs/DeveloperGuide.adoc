...
Each `Task` can have an unique `ModuleCode` tag,
...
*Pros*: Low coupling with the `Module` class. The `Module` does not need to know that there are a list of `Task` s for it.
...
Alternative 1 is chosen for easier implementation, and to reduce coupling
between `Task` and `Module`.

===== Aspect: A `Task` may have a specified time period, or no time period. How do we implement this feature?
* **Alternative 1 (current choice):**

`Task` is implemented as an abstract class. A task with a specified time period will be created as a `ScheduledTask`, while a task with no
time period will be created as a `NonScheduledTask`. `ScheduledTask` and
...
`task *edit*` allows us to add a time period to a class. In this case, our `Task` is
initially a `NonScheduledTask`, so we have to re-create a new `ScheduledTask`
with the same description, and the specified time period.

* **Alternative 2:**

`Task` is implemented as a concrete class. `Task` s without a specified time period
will have  `taskDateTime` set to `null`, while `Task` s with a given time period will be assign
